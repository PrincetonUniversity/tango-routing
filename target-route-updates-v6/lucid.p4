...
control IngressControl(inout hdr_t hdr,
  ...
  bit<8> route_manager_0_idx_2147;
  ...
  action labeledstmt_40(){
    route_manager_0_idx_2147=hdr.update_route.update_route_update_0;
  }
  ...
  action labeledstmt_42(){
    route_manager_0_idx_2147=traffic_class;
  }
  ...
  route_manager_0_regaction_2148 = {
...
RouteManager_get_path_id_ret=route_manager_0_regaction_2148.execute(route_manager_0_idx_2147);
  }
  action labeledstmt_10(){
    labeledstmt_43();
  }
  RegisterAction<bit<8>,bit<8>,bit<8>>(route_manager_0)
  route_manager_0_regaction_2149 = {
    void apply(inout bit<8> cell1_remote,
        out bit<8> ret_remote){
      bit<8> cell1_local=cell1_remote;
      bit<8> cell2_local=0;
  ...
  action labeledstmt_44(){
    route_manager_0_regaction_2149.execute(route_manager_0_idx_2147);
  }
...
forward_tango_pkt_arg_13=(hdr.forward_flow.forward_flow_ip_header_1+16w58);
  ...
  sequence_counters_0_regaction_2150 = {
...
SequenceNumberManager_increment_ret=sequence_counters_0_regaction_2150.execute(path_id);
  }
  bit<12> timestamp;
...
hdr.incoming_tango_traffic.incoming_tango_traffic_encaped_udp_header_3=forward_tango_pkt_arg_31;
  ...
  table table_2162 {
    ...
    const entries = {
      (2,_,_,_,_,_,_,_) : labeledstmt_1();
      (1,_,_,_,_,_,_,_) : labeledstmt_2();
      (3,_,_,_,_,_,_,_) : labeledstmt_3();
  ...
  table table_2159 {
    ...
    const entries = {
      ...
      (_,_,_,_,_,_,_,3) : labeledstmt_11();
  ...
  table table_2158 {
    ...
    const entries = {
      (2,_,_,_,_,_,_,_) : labeledstmt_12();
      (1,_,_,_,_,_,_,_) : labeledstmt_13();
      (3,_,_,_,_,_,_,_) : labeledstmt_14();
  ...
  table table_2157 {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_15();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_16();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_15();
    } 
  } 
  table table_2156 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_17();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_18();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_17();
    } 
  } 
  table table_2155 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_19();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_20();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_19();
    } 
  } 
  table table_2154 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_21();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_22();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_21();
    } 
  } 
  table table_2153 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_23();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_24();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_23();
    } 
  } 
  table table_2152 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_25();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_26();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_25();
    } 
  } 
  table table_2151 {
    key = {
    ...
    const entries = {
      (_,_,_,_,_,_,_,_,2) : labeledstmt_27();
      (_,_,_,_,_,_,_,_,1) : labeledstmt_28();
      (_,_,_,_,_,_,_,_,_) : labeledstmt_27();
    } 
  } 
  apply {
    table_2162.apply();
