...
def parse_header(data, HdrDef):
    ...
    if len(data) < struct.calcsize(HdrDef.fmt):
        return None, []
    return (
        HdrDef(*struct.unpack(HdrDef.fmt, data[0 : struct.calcsize(HdrDef.fmt)])),
        data[struct.calcsize(HdrDef.fmt) :],
    )
