...
def pktFromRec(rec):
    ...
    if rec["eth.type"] == dpkt.ethernet.ETH_TYPE_IP:
        if rec["ip.proto"] == dpkt.ip.IP_PROTO_UDP:
            ...
            udpPkt = dpkt.udp.UDP(sport=int(rec["udp.src"]), dport=int(rec["udp.dst"]))
            udpPkt.data = payload
    ...
    ethFrame = dpkt.ethernet.Ethernet(
        src=hexStrToBin(rec["eth.src"]), dst=hexStrToBin(rec["eth.dst"]), type=int(rec["eth.type"])
    )
