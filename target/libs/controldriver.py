...
class Controller(object):
    ...
    def add_multicast_group(self, mc_gid, ports_rids):
        ...
        print(
            "[add_multicast_group] adding mc group: {0}--> [{1}]".format(
                str(mc_gid), str(ports_rids)
            )
        )
...
class BfRtTable:
    ...
    def load_key(self):
        ...
        for idx, field_id in enumerate(field_ids):
            field_name = self._cintf.key_field_name_get(self._handle, field_id).value.decode(
                "ascii"
            )
            ...
            readable = "{!s:30} type={!s:10} size={:^2}".format(
                field_name, key_type_map(field_type), field_size
            )
    ...
    def load_field_annotation(self, field_id, action_id):
        ...
        if action_id == None:
            ...
            annotations_arr = self._cintf.data_field_annotations_with_action_get_array(
                self._handle, field_id, action_id
            )
        return [(ann.name.decode("ascii"), ann.value.decode("ascii")) for ann in annotations_arr]

    def load_data_field(
        self, data_fields, idx, field_id, action_id=None, action_name=None, depth=0
    ):
        ...
        if not action_id:
            action_id = c_uint(0)
        field_type = self._cintf.data_field_type_with_action_get(
            self._handle, field_id, action_id
        ).value
        field_name = self._cintf.data_field_name_with_action_get(
            self._handle, field_id, action_id
        ).value.decode("ascii")
        field_size = self._cintf.data_field_size_with_action_get(
            self._handle, field_id, action_id
        ).value
        field_is_ptr = self._cintf.data_field_is_ptr_with_action_get(
            self._handle, field_id, action_id
        ).value
        read_only = self._cintf.data_field_is_read_only_with_action_get(
            self._handle, field_id, action_id
        ).value
        mandatory = self._cintf.data_field_is_mandatory_with_action_get(
            self._handle, field_id, action_id
        ).value
        readable = "{!s:30} type={!s:10} size={:^2}".format(
            field_name, data_type_map(field_type), field_size
        )
        ...
        if data_type_map(field_type) == "CONTAINER":
            ...
            for idx, c_field_id in enumerate(c_fields):
                self.load_data_field(
                    data_fields[field_name]["cont_data_fields"], idx, c_field_id, depth=depth + 1
                )
                data_fields[field_name]["cont_data_fields"] = ensure_sorted(
                    data_fields[field_name]["cont_data_fields"]
                )
            # print ("***")
        return 0
    ...
    def load_data(self):
        ...
        if len(self.actions) == 0:
            ...
            for action_name, action_info in self.actions.items():
                field_ids = self._cintf.data_field_list_with_action_get_array(
                    self._handle, action_info["id"]
                )
                for idx, field_id in enumerate(field_ids):
                    # note that we pass the action_info dict because
                    # it may contain nested fields that get filled recursively.
                    self.load_data_field(
                        action_info["data_fields"], idx, field_id, action_info["id"], action_name
                    )
            ...
            if key_type_map(field_info["type"]) == "EXACT":
                if data_type_map(field_info["data_type"]) != "String":
                    if not field_info["is_ptr"]:
                        ...
                        self._cintf.key_field_set_value(
                            key_hdl, field_info["id"], c_ulonglong(field_val)
                        )
                        continue
                    else:
                ...
                if data_type_map(field_info["data_type"]) != "String":
                    if not field_info["is_ptr"]:
                        ...
                        self._cintf.key_field_set_value_and_mask(
                            key_hdl, field_info["id"], value, mask
                        )
                        continue
                    else:
                        print("unexpected: key field pointer")
    ...
    def fill_data_fields(self, data_hdl, args, fields_info):
        ...
        for field_name, field_val in args.items():
            ...
            if data_type == "BYTE_STREAM":
                ...
                dprint(
                    "[fill_data_fields / BYTE_STREAM] field: {0} value: {1} n_bytes: {2}".format(
                        field_name, cval, n_bytes
                    )
                )
                ...
                for idx, v in enumerate(field_val):
                    value[idx] = v
                self._cintf.data_field_set_value_bool_array(
                    data_hdl, field_info["id"], value, arrlen
                )
            else:
    ...
    def create_action(self, action_name, action_args):
        ...
        """
        # ref set_data_fields
        action_hdl = self._cintf.table_action_data_allocate(
            self._handle, self.actions[action_name]["id"]
        )
        succ = self.fill_data_fields(
            action_hdl, action_args, self.actions[action_name]["data_fields"]
        )
        # ref _set_data_field
    ...
    def cintf_table_get(self, key_hdl, data_hdl):
        ...
        return self._cintf.table_entry_get(
            ...
            self._cintf.get_session(),
            self._cintf.get_dev_tgt(),
            1,  # 1 is flag to read from hardware
            key_hdl,
            data_hdl,
        )
            ...
            if is_active:
                if dtype == "BYTE_STREAM":
                    ...
                    if ("$bfrt_field_class", "register_data") in info["annotations"]:
                        raw_field_vals = self._cintf.data_field_get_value_u64_array(
                            data_handle, info["id"]
                        )
    ...
    def del_entry(self, key_handle):
        ...
        sts = self._cintf.table_entry_del(
            self._handle, self._cintf.get_session(), self._cintf.get_dev_tgt(), 0, key_handle
        )
        if sts != 0:
...
class LibcInterface(object):
    ...
    def wrap_clib_fcns(self):
        ...
        fcn_sigs = [
            ...
            (
                "table_has_const_default_action",
                c_bool,
                (False,),
                self._driver.bf_rt_table_has_const_default_action,
            ),
            ...
            (
                "data_field_name_with_action_get",
                c_char_p,
                (),
                self._driver.bf_rt_data_field_name_with_action_get,
            ),
            (
                "data_field_type_with_action_get",
                c_int,
                (-1,),
                self._driver.bf_rt_data_field_type_with_action_get,
            ),
            (
                "data_field_size_with_action_get",
                c_uint,
                (),
                self._driver.bf_rt_data_field_size_with_action_get,
            ),
            (
                "data_field_is_ptr_with_action_get",
                c_bool,
                (),
                self._driver.bf_rt_data_field_is_ptr_with_action_get,
            ),
            (
                "data_field_is_read_only_with_action_get",
                c_bool,
                (),
                self._driver.bf_rt_data_field_is_read_only_with_action_get,
            ),
            (
                "data_field_is_mandatory_with_action_get",
                c_bool,
                (),
                self._driver.bf_rt_data_field_is_mandatory_with_action_get,
            ),
            ...
            (
                "table_action_data_allocate",
                self.handle_type,
                (),
                self._driver.bf_rt_table_action_data_allocate,
            ),
        ...
        method_sigs = [
            ...
            (
                "bf_rt_container_data_field_list_get",
                self._driver.bf_rt_container_data_field_list_get,
            ),
        ...
        array_fcn_sigs = [
            ...
            (
                "data_field_list_with_action_get_array",
                c_uint,
                (),
            ...
            (
                "data_field_annotations_get_array",
            ...
            (
                "data_field_annotations_with_action_get_array",
                self.annotation_type,
                (),
            ...
            (
                "data_field_get_value_u64_array",
                c_ulonglong,
                (),  # args: (data_handle, field_id)
