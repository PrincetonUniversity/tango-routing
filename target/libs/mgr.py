...
class Manager(object):
    ...
    def grpc_connect(self):
        if self.p4Name != None:
            ...
            self.interface = gc.ClientInterface(
                grpc_addr, client_id=client_id, device_id=0, notifications=None
            )
    ...
    def fixed_function_connect(self):
        ...
        self.conn_mgr = conn_mgr_client_module.Client(
            TMultiplexedProtocol.TMultiplexedProtocol(bprotocol, "conn_mgr")
        )
        self.mc = mc_client_module.Client(
            TMultiplexedProtocol.TMultiplexedProtocol(bprotocol, "mc")
        )
    ...
    def add_mc_group(self, mc_gid, dpids):
        ...
        mc_grp_hdl = self.mc.mc_mgrp_create(
            self.mc_sess_hdl, self.dev_tgt.dev_id, hex_to_i16(mc_gid)
        )
        mc_node_hdl = self.mc.mc_node_create(
            self.mc_sess_hdl, self.dev_tgt.dev_id, 0, port_map, lag_map
        )
        self.mc.mc_associate_node(
            self.mc_sess_hdl, self.dev_tgt.dev_id, mc_grp_hdl, mc_node_hdl, 0, 0
        )
    ...
    def add_multinode_mc_group(self, mc_gid, dpids_and_rids):
        ...
        for (dpid, rid) in dpids_and_rids:
            ...
            mc_node_hdl = self.mc.mc_node_create(
                self.mc_sess_hdl, self.dev_tgt.dev_id, rid, port_map, lag_map
            )
            self.mc.mc_associate_node(
                self.mc_sess_hdl, self.dev_tgt.dev_id, mc_grp_hdl, mc_node_hdl, 0, 0
            )
    ...
    def addExactEntry(self, tableName, fieldNames, fieldVals, actionName, actionArgs={}):
        print(
            "adding exact entry: {0}[{1}=={2}] --> {3}({4})".format(
                tableName, str(fieldNames), str(fieldVals), actionName, actionArgs
            )
        )
        ...
        key_list = [
            table.make_key([gc.KeyTuple(fieldName, fieldVal)])
